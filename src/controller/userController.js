...
const userRegister = async function (req, res) {
    try {
        ...
        if (Object.keys(data).length == 0) return res.status(400).send({ status: false, message: "Please Enter data" })  //{} []=0
...
const userLogin = async function (req, res) {
    ...
    const Login = await userModel.findOne({ email })      /////store entire schema
